...
<div class="notification-node" class:selected class:error={hasError} class:pending={executionState === 'pending'} class:running={executionState === 'running'} class:completed={executionState === 'completed'} class:failed={executionState === 'failed'} class:skipped={executionState === 'skipped'}>
  <button
    class="delete-btn"
...
<style>
  ...
  .notification-node:hover .delete-btn,
  .notification-node.selected .delete-btn {
    opacity: 1;
  }
  ...
  .notification-node.selected {
    border-color: #ef4444;
    box-shadow: 0 0 0 3px rgba(239, 68, 68, 0.2);
  }

  .notification-node:hover {
    border-color: #ef4444;
  ...
  /* Execution States */
  .notification-node.pending {
    border-color: #9ca3af;
    border-style: dashed;
  }

  .notification-node.running {
    border-color: #3b82f6;
